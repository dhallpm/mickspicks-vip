const WEEKLY_RESULTS = [
  { Date:'2026-06-24', Sport:'MLB', League:'MLB', Game:'Braves at Padres', Pick:'Braves ML', 'Bet Type':'Moneyline', Odds:'-115', Grade:'A-', Units:0.75, Result:'Loss', 'Profit/Loss':'-0.75u', Access:'VIP', Writeup:'Padres 5, Braves 2. Braves moneyline lost.' },
  { Date:'2026-06-24', Sport:'MLB', League:'MLB', Game:'Dodgers at Twins', Pick:'Dodgers ML', 'Bet Type':'Moneyline', Odds:'-172', Grade:'B+', Units:0.75, Result:'Win', 'Profit/Loss':'+0.44u', Access:'VIP', Writeup:'Dodgers 4, Twins 3. Dodgers moneyline cashed.' },
  { Date:'2026-06-24', Sport:'MLB', League:'MLB', Game:'Brewers at Reds', Pick:'Brewers ML', 'Bet Type':'Moneyline', Odds:'-128', Grade:'B+', Units:0.5, Result:'Win', 'Profit/Loss':'+0.39u', Access:'Free', Writeup:'Brewers 6, Reds 5. Brewers moneyline cashed.' },
  { Date:'2026-06-24', Sport:'MLB', League:'MLB', Game:'Diamondbacks at Cardinals', Pick:'Diamondbacks ML', 'Bet Type':'Moneyline', Odds:'+100', Grade:'B', Units:0.5, Result:'Win', 'Profit/Loss':'+0.50u', Access:'Free', Writeup:'Diamondbacks 9, Cardinals 4. Arizona moneyline cashed.' },
  { Date:'2026-06-24', Sport:'MLB', League:'MLB', Game:'Royals at Rays', Pick:'Under 8', 'Bet Type':'Total', Odds:'-136', Grade:'B', Units:0.5, Result:'Push', 'Profit/Loss':'0.00u', Access:'Free', Writeup:'Rays 5, Royals 3. Total landed 8, under 8 pushed.' },
  { Date:'2026-06-24', Sport:'WNBA', League:'WNBA', Game:'Lynx at Mystics', Pick:'Lynx -9.5', 'Bet Type':'Spread', Odds:'-110', Grade:'B', Units:0.5, Result:'Loss', 'Profit/Loss':'-0.50u', Access:'Free', Writeup:'Lynx won 78-76 but did not cover -9.5.' },
  { Date:'2026-06-24', Sport:'WNBA', League:'WNBA', Game:'Dream at Valkyries', Pick:'Valkyries +2.5', 'Bet Type':'Spread', Odds:'-110', Grade:'B', Units:0.5, Result:'Win', 'Profit/Loss':'+0.45u', Access:'Free', Writeup:'Valkyries won outright 77-66. Golden State +2.5 cashed.' },
...
function resultOf(row) { return String(row.Result || '').trim() }
...
export default async function handler(req, res) {
  ...
  const units = rows.reduce((sum, row) => sum + unitValue(row), 0)
  res.setHeader('Cache-Control', 'no-store, no-cache, must-revalidate, max-age=0')
  res.status(200).json({
    success: true,
    source: 'weekly-results-static-june24-current',
    ...
    results: rows,
    allRows: rows,
    vip: rows.filter(row => /vip|premium|member/i.test(String(row.Access || '')))
  })
}
